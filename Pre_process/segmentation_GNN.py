...
def read_data(path, file):
    ...
    for item in data:
        ...
        #st = [time, str(item[5]),int(item[2]), float(item[3]), float(item[4]) ]
        st = [time, str(item[4]) , float(item[2]), float(item[3])]
...
def segment_interval_split(user_id, user_list, interval):
    ...
    for i in range(1,len(split_index)):    
        sub_trajectory.append(sub[split_index[i-1]:split_index[i]])
        
    sub_trajectory.append(sub[split_index[-1]:])
...
def segmentation_hour(data_user, hour):
    ...
    for user in data_user.keys():
        ...
        for i in range(0,len(ind)-1):
            ...
            user_sub.append([user]+sub)
            
        for item in user_sub:
            sub_trajectory.append(item)
    ...
    for user in data_user.keys():
        ...
        for item in data_user[user]:
            if item[0].date().strftime('%y %m %d') in data_date.keys():
                st=[item[1],item[2],item[3]]
                data_date[item[0].date().strftime('%y %m %d')].append(st)
...
def write_dat(path,file,subs):
    ...
    for element in subs:
        ...
        for x in range(len(element[-1])-1):
            textfile.write(str(element[-1][x])+' ')
        textfile.write(str(element[-1][-1])+'\n')

    textfile.close()
    
    # for_vec = open(path+file.split('_',1)[0]+'_onlyPOI'+'.dat', "w")
    # for element in subs:
    #     for_vec.write(str(element[0])+' ')
    #     for i in range(1,len(element)-1):
    #         for_vec.write(str(element[i][0])+' ')
    #     for_vec.write(str(element[-1][0])+'\n')
...
if __name__ == '__main__':
    ...
    print('the segmentation method you choiced is [ {} ], processing...'.format(segmentation_method))
...
#output the segmented data     
    output_path = './data/segmented_data/seg_GNN/'  
    file_name = file.split('_')[0]+'_'+str(segmentation_method)+'_'+str(time_interval)+'GNN'
