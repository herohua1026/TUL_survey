...
def read_train_data():
    ...
    test_UserT = list()
    test_lens = list()
    ftraindata = open(data_input_path+data_input, 'r')  # gowalla_scopus_1006.dat,Gowalla_segmentation_6h
    tempT = list()  #
    pointT = list()  #
    userT = list()  # User ID
    seqlens = list()  #
   ...
   # print("userT length=",len(User_List))
    ...
    temp_seqlens = list()
    User = 0  #
    rate = 0.2  # 10% for test

    #于一个用户的轨迹，找到最后的10%作为test，剩下的作为train，用户id为70的只有一条轨迹，无法分割test，因此只有train
...
def get_xs():  #
    ...
    """
    fpointvec = open(vec_input_path+vec_input, 'r')  #gowalla_vector_250d.dat  gowalla_em_250.dat
    #     table_X={}  #
    item = 0
